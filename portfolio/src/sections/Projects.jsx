...
const projects = [
  ...
  },
  {
    ...
    stack: ['Python','PyTorch','EfficientNet','FastAPI','React (Vite)','OpenCV','Node.js'],
    github: '#',
  },
  {
    id: 3,
    ...
    stack: ['Node.js','TypeScript','React (Vite)','Express.js','PostgreSQL','Zod','NPM'],
    github: '#',
  },
  {
    id: 4,
...
function ProjectCard({ project, inView, delay }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55, delay }}
    ...
    >
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h3 className="font-display font-bold text-xl text-text mb-1">{project.name}</h3>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-body text-xs font-semibold" style={{ color: project.color }}>{project.tag}</span>
          ...
          <div className="flex items-center gap-2.5 flex-shrink-0 mt-1">
            {project.github && (
              <motion.a href={project.github} whileHover={{ scale: 1.12 }} className="text-muted hover:text-primary2 transition-colors">
                <Github size={16} />
              </motion.a>
            )}
            {project.live && (
              <motion.a href={project.live} whileHover={{ scale: 1.12 }} className="text-muted hover:text-primary2 transition-colors">
        ...
        <p className="font-body text-sm text-muted leading-relaxed mb-4">{project.desc}</p>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
              ...
              <ul className="space-y-2 mb-4">
                {project.highlights.map((h, i) => (
                  <li key={i} className="flex gap-2.5 text-sm text-textDim leading-relaxed">
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-32 px-6 relative">
      ...
      <div className="absolute top-1/2 left-0 w-[300px] h-[300px] pointer-events-none"
      ...
      <div ref={ref} className="section-content max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55 }}>
          <SectionHeading number="04" label="Selected Projects" title={"Things I've\nBuilt"} />
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {projects.map((p, i) => <ProjectCard key={p.id} project={p} inView={inView} delay={i*0.1} />)}
        </div>
      </div>
    </section>
  )
